...
const SideNav: React.FC<SiderProps> = React.memo(({ ...others }) => {
  const { user } = useAuth();
  ...
  const menuItems = useMemo<MenuProps["items"]>(() => {
    ...
    if (user?.role === "admin") {
      items.push(
        ...
        getItem(
          <Link to={PATH_CUSTOMFIELD.root}>Custom Fields</Link>,
          "customfields",
          <PicCenterOutlined />
        ),
  ...
  const onOpenChange = useCallback(
    (keys: any) => {
      const latest = keys.find((k: any) => !openKeys.includes(k));
      setOpenKeys(latest && rootSubmenuKeys.includes(latest) ? [latest] : keys);
    },
    [openKeys]
  );
